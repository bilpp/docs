...
On the bright side, this overdue upgrade brings us closer to the conventions of best practices of [Mithril.js](https://mithril.js.org/), the mini-framework used for Bilpp's UI.
...
::: tip
If you need help with the upgrade, our friendly community will gladly help you out either [on the forum](https://discuss.bilpp.com/t/extensibility) or [in chat](https://bilpp.com/chat/).
:::
...
#### m.prop -> `bilpp/utils/Stream`

Mithril streams, which were available via `m.prop` in Mithril 0.2, are now available via `bilpp/utils/Stream`. `m.prop` will still work for now due to a temporary BC layer.

#### m.withAttr -> withAttr

The `m.withAttr` util has been removed from Mithril. We have provided `bilpp/utils/withAttr`, which does the same thing. A temporary BC layer has been added for `m.withAttr`.
...
Please note that if your component is extending Bilpp's helper `Component` class, you must call `super.METHOD(vnode)` if using `oninit`, `oncreate`, and `onbeforeupdate`.
...
// Mithril 2
import Link from 'bilpp/components/Link';
...
extending Bilpp's `Component` helper class. The best workaround here is to just use another name for this attr.
...
- `m.prop` -> `bilpp/utils/Stream`
- `m.withAttr` -> `bilpp/utils/withAttr`
- `moment` -> `dayjs`
...
```json
{
  ...
  "license": "MIT",
  "require": {
    "bilpp/core": "^0.1.0-beta.14",
    "bilpp/tags": "^0.1.0-beta.14",  // Will mark tags as a dependency
    "bilpp/mentions": "^0.1.0-beta.14",  // Will mark mentions as a dependency
  }
  // other config
}
```
...
Although Bilpp uses multiple components of the Laravel framework, it is not a pure Laravel system. In beta 14, the `Bilpp\Foundation\Application` class no longer implements `Illuminate\Contracts\Foundation\Application`, and no longer inherits `Illuminate\Container\Container`. Several things to note:

- The `app` helper now points to an instance of `Illuminate\Container\Container`, not `Bilpp\Foundation\Application`. You might need to resolve things through the container before using them: for instance, `app()->url()` will no longer work; you'll need to resolve or inject an instance of `Bilpp\Foundation\Config` and use that.
- Injected or resolved instances of `Bilpp\Foundation\Application` can no longer resolve things through container methods. `Illuminate\Container\Container` should be used instead.
- Not all public members of `Illuminate\Contracts\Foundation\Application` are available through `Bilpp\Foundation\Application`. Please refer to our [API docs on `Bilpp\Foundation\Application`](https://api.docs.bilpp.com/php/master/bilpp/foundation/application) for more information.
...
- `Bilpp/Foundation/Config` and `Bilpp/Foundation/Paths` can now be injected where needed; previously their data was accessible through `Bilpp/Foundation/Application`.

### Deprecations

- `url` provided in `config.php` is now an array, accessible via `$config->url()`, for an instance of `Config` - [PR](https://github.com/bilpp/core/pull/2271#discussion_r475930358)
- AssertPermissionTrait has been deprecated - [Issue](https://github.com/bilpp/core/issues/1320)
- Global path helpers and path methods of `Application` have been deprecated, the injectable `Paths` class should be used instead - [PR](https://github.com/bilpp/core/pull/2155)
- `Bilpp\User\Event\GetDisplayName` has been deprecated, the `displayNameDriver` method of the `User` extender should be used instead - [PR](https://github.com/bilpp/core/pull/2174)
...
- app()->url() will no longer work: [`Bilpp\Http\UrlGenerator`](routes.md) should be injected and used instead. An instance of `Bilpp\Http\UrlGenerator` is available in `blade.php` templates via `$url`.
...
- The following events deprecated in Beta 13 [have been removed](https://github.com/bilpp/core/commit/7d1ef9d89161363d1c8dea19cf8aebb30136e9e3#diff-238957b67e42d4e977398cd048c51c73):
