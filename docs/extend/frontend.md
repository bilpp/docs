...
This page describes how to make changes to Bilpp's user interface. How to add buttons, marquees, and blinking text. 🤩

[Remember](/extend/start.md#architecture), Bilpp's frontend is a **single-page JavaScript application**. There's no Twig, Blade, or any other kind of PHP template to speak of. The few templates that are present in the backend are only used to render search-engine-optimized content. All changes to the UI need to be made via JavaScript.

Bilpp has two separate frontend applications:

* `forum`, the public side of your forum where users create discussions and posts.
* `admin`, the private side of your forum where, as an administrator of your forum, you configure your Bilpp installation.

They share the same foundational code, so once you know how to extend one, you know how to extend both.

:::tip Typings!
Use our new [Typing Library](https://www.npmjs.com/package/bilpp) as a dev dependency for editor autocomplete to make frontend development easier!
:::

## Transpilation and File Structure

This portion of the guide will explain the necessary file setup for extensions. Once again, we highly recommend using the unofficial [FoF extension generator](https://github.com/FriendsOfBilpp/extension-generator) to set up the file structure for you. That being said, you should still read this to understand what's going on beneath the surface.

Before we can write any JavaScript, we need to set up a **transpiler**. This allows us to use [TypeScript](https://www.typescriptlang.org/) and its magic in Bilpp core and extensions.
...
This can be tricky because everyone's system is different. From the OS you're using, to the program versions you have installed, to the user access permissions – I get chills just thinking about it! If you run into trouble, ~~tell him I said hi~~ use [Google](https://google.com) to see if someone has encountered the same error as you and found a solution. If not, ask for help from the [Bilpp Community](https://discuss.bilpp.com) or on the [Discord chat](https://bilpp.com/discord/).
...
```json
{
  ...
  "dependencies": {
    "bilpp-webpack-config": "0.1.0-beta.10",
    "webpack": "^4.0.0",
    "webpack-cli": "^3.0.7"
  },
  "dev-dependencies": {
...
Please note that we do not need to include `bilpp/core` or any bilpp extensions as dependencies: they will be automatically packaged when Bilpp compiles the frontends for all extensions.

### webpack.config.js

```js
const config = require('bilpp-webpack-config');
...
To work properly, our extensions should use the [official bilpp webpack config](https://github.com/bilpp/bilpp-webpack-config) (shown in the above example).
...
```js
import {extend, override} from 'bilpp/common/extend';
...
Pretty much every Bilpp extension will need to import *something* from Bilpp Core.
Like most extensions, core's JS source code is split up into `admin`, `common`, and `forum` folders. You can import the file by prefixing its path in the Bilpp core source code with `bilpp`. So `admin/components/AdminLinkButton` is available as `bilpp/admin/components/AdminLinkButton`, `common/Component` is available as `bilpp/common/Component`, and `forum/states/PostStreamState` is available as `bilpp/forum/states/PostStreamState`.

In some cases, an extension may want to extend code from another bilpp extension. This is only possible for extensions which explicitly export their contents.

* `bilpp/tags` and `bilpp/flags` are currently the only bundled extensions that allow extending their JS. You can import their contents from `bilpp/{EXT_NAME}/PATH` (e.g. `bilpp/tags/components/TagHero`).
...
In order for your extension's JavaScript to be loaded into the frontend, we need to tell Bilpp where to find it. We can do this using the `Frontend` extender's `js` method. Add it to your extension's `extend.php` file:
...
Bilpp will make anything you `export` from `forum.js` available in the global `bilpp.extensions['acme-hello-world']` object. Thus, you may choose to expose your own public API for other extensions to interact with.
...
::: tip
You should develop extensions with debug mode turned **on** in `config.php`. This will ensure that Bilpp recompiles assets automatically, so you don't have to manually clear the cache every time you make a change to your extension JavaScript.
:::

## Changing the UI Part 1

Bilpp's interface is constructed using a JavaScript framework called [Mithril.js](https://mithril.js.org/). If you are familiar with [React](https://reactjs.org), then you'll catch on in no time. But if you are not familiar with any JavaScript frameworks, we suggest you go through a [tutorial](https://mithril.js.org/simple-application.html) to understand the fundamentals before proceeding.

The crux of it is that Bilpp generates virtual DOM elements which are a JavaScript representation of HTML. Mithril takes these virtual DOM elements and turns them into real HTML in the most efficient way possible. (That's why Bilpp is so speedy!)
...
import HeaderPrimary from 'bilpp/forum/components/HeaderPrimary';
...
Bilpp's interface is made up of many nested **components**. Components are a bit like HTML elements in that they encapsulate content and behavior. For example, look at this simplified tree of the components that make up a discussion page:
...
To use Bilpp components, simply extend `bilpp/common/Component` in your custom component class.
...
import DiscussionPage from 'bilpp/forum/components/DiscussionPage';
...
...will turn Bilpp's discussion pages into proclamations of "Hello World". How creative!

In most cases, we don't actually want to completely replace the methods we are modifying. For this reason, Bilpp includes `extend` and `override` utils. `extend` allows us to add code to run after a method has completed. `override` allows us to replace a method with a new one, while keeping the old method available as a callback. Both are functions that take 3 arguments:
...
import { extend, override } from 'bilpp/common/extend';
import HeaderPrimary from 'bilpp/forum/components/HeaderPrimary';
import ItemList from 'bilpp/common/utils/ItemList';
...
Since all Bilpp components and utils are represented by classes, `extend`, `override`, and regular old JS mean that we can hook into, or replace, ANY method in any part of Bilpp.
Some potential "advanced" uses include:

* Extending or overriding `view` to change (or completely redefine) the html structure of Bilpp components. This opens Bilpp up to unlimited theming
* Hooking into Mithril component methods to add JS event listeners, or otherwise redefine business logic.

### Bilpp Utils

Bilpp defines (and provides) quite a few util and helper functions, which you may want to use in your extensions. The best way to learn about them is through [the source code](https://github.com/bilpp/core/tree/master/js) or [our javascript API documentation](https://api.docs.bilpp.com/js/).
