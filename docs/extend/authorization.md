...
As with any framework, Bilpp allows certain actions and content to be restricted to certain users. There are 2 parallel systems for this:
...
Authorization queries are made with 3 parameters, with logic contained in [`Bilpp\User\Gate`](https://api.docs.bilpp.com/php/master/bilpp/user/gate):
...
- `Bilpp\User\Access\AbstractPolicy::ALLOW` (via `$this->allow()`)
- `Bilpp\User\Access\AbstractPolicy::DENY` (via `$this->deny()`)
- `Bilpp\User\Access\AbstractPolicy::FORCE_ALLOW` (via `$this->forceAllow()`)
- `Bilpp\User\Access\AbstractPolicy::FORCE_DENY` (via `$this->forceDeny()`)
...
Bilpp's authorization system is accessible through public methods of the `Bilpp\User\User` class. The most important ones are listed below; others are documented in our [PHP API documentation](https://api.docs.bilpp.com/php/master/bilpp/user/user).


In this example, we will use `$actor` as an instance of `Bilpp\User\User`, `'viewDiscussions'` and `'reply'` as examples of abilities, and `$discussion` (instance of `Bilpp\Discussion\Discussion`) as an example argument.
...
Let's take a look at an example policy from [Bilpp Tags](https://github.com/bilpp/tags/blob/master/src/Access):

```php
<?php
namespace Bilpp\Tags\Access;

use Bilpp\Tags\Tag;
use Bilpp\User\Access\AbstractPolicy;
use Bilpp\User\User;
...
use Bilpp\Settings\SettingsRepositoryInterface;
use Bilpp\Tags\Tag;
use Bilpp\User\Access\AbstractPolicy;
use Bilpp\User\User;
...
We do this via the `whereVisibleTo` method, which is defined in `Bilpp\Database\ScopeVisibilityTrait`, and available to [Eloquent models and queries](https://laravel.com/docs/6.x/queries) through [Eloquent scoping](https://laravel.com/docs/6.x/eloquent#local-scopes).
For example:

```php
use Bilpp\Group\Group;
...
Please note that visibility scoping can only be used on models that use the `Bilpp\Database\ScopeVisibilityTrait` trait.

### How It's Processed

So, what actually happens when we call `whereVisibleTo`?
This call is handled by Bilpp's general model visibility scoping system, which runs the query through a sequence of callbacks, which are called "scopers".

The query will be run through all applicable scopers registered for the model of the query. Note that visibility scopers registered for a parent class (like `Bilpp\Post\Post`) will also be applied to any child classes (like `Bilpp\Post\CommentPost`).
...
Let's take a look at an annotated, simple piece of `Bilpp\Post\PostPolicy` as an example:
...
Let's take a look at some examples from [Bilpp Tags](https://github.com/bilpp/tags/blob/master/src/Access/TagPolicy).
...
```php
use Bilpp\Extend;
use Bilpp\Discussion\Discussion;
use Bilpp\Tags\Tag;
use YourNamespace\Access;
