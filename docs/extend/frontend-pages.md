...
We provide `bilpp/common/components/Page` as a base class for pages in both the `admin` and `forum` frontends. It has a few benefits:
...
```js
import Page from 'bilpp/common/components/Page';
...
Bilpp uses a setting to determine which page should be the homepage: this gives admins flexibility to customize their communities.
...
An example from the [Tags extension](https://github.com/bilpp/tags/blob/master/js/src/admin/addTagsHomePageOption.js):

```js
import { extend } from 'bilpp/common/extend';
import BasicsPage from 'bilpp/common/components/BasicsPage';

export default function() {
  extend(BasicsPage.prototype, 'homePageItems', items => {
    items.add('tags', {
      path: '/tags',
      label: app.translator.trans('bilpp-tags.admin.basics.tags_label')
    });
  });
}
```
...
To allow this, Bilpp creates (and stores) instances of [`PageState`](https://api.docs.bilpp.com/js/master/class/src/common/states/pagestate.js~pagestate) as `app.current` and `app.previous`.
These store:
...
For example, this is how the Discussion Page makes its [`PostStreamState`](https://api.docs.bilpp.com/js/master/class/src/forum/states/poststreamstate.js~poststreamstate) instance globally available.
...
```jsx
import IndexPage from 'bilpp/forum/components/DiscussionPage';
...
Bilpp actually already wraps all its components in the `bilpp/common/resolvers/DefaultResolver` resolver. This has the following benefits:
...
- the `resolver` key can be used to provide an **instance** of a route resolver. This instance should define which component should be used, and hardcode the route name to be passed into it. This instance will be used without any modifications by Bilpp.
- The `resolverClass` key AND `component` key can be used to provide a **class** that will be used to instantiate a route resolver, to be used instead of Bilpp's default one, as well as the component to use. Its constructor should take 2 arguments: `(component, routeName)`.
...
// Use the default resolver class (`bilpp/common/resolvers/DefaultResolver`)
app.routes['resolverClass'] = {path: '/custom/path/2', component: CustomPage};
```

### Custom Resolvers

We strongly encourage custom route resolvers to extend `bilpp/common/resolvers/DefaultResolver`.
For example, Bilpp's `bilpp/forum/resolvers/DiscussionPageResolver` assigns the same key to all links to the same discussion (regardless of the current post), and triggers scrolling when using `m.route.set` to go from one post to another on the same discussion page:
